...
Main.startAnimation = function() {
    ...
    try {
        if (__START_INTERACTION_IMMEDIATELY === true) Main.animationFinished();
    } catch (e) {}
}
Main.animationFinished = function(delay) {
    ...
    if (Main.animationFinished.called == true) return;
...
Main.init = function() {
    ...
    Main.interaction = $('.etkilesimalan')
        .get(0);
    Main.animation = $('.ornek')
        .get(0);
    Main.objective = $('.mavikontrol')
        .get(0);
    ...
    if (framework == 'raphael') {
        ...
        var interactionWidth = $(Main.interaction)
            .width();
        var interactionHeight = $(Main.interaction)
            .height();
        ...
        var animationWidth = $(Main.animation)
            .width();
        var animationHeight = $(Main.animation)
            .height();
        ...
        try {
            if (__START_INTERACTION_IMMEDIATELY === true) Main.startAnimation();
            else throw '';
        } catch (e) {
...
Main.initializeNavigation = function() {
    var createWordList = function(letter) {
        ...
        $('.navlink')
            .removeClass('harfselected');
        $('.navlink[data-letter="' + letter + '"]')
            .addClass('harfselected');
        ...
        if (change > 100) {
            ...
            animHelper.animate({
                ...
                update: function() {
                    $(div)
                        .css({
                        backgroundPosition: (this.change - 100) + 'px -9px'
                    });
                },
                callback: function() {
                    $(div)
